...
mkf obs24
mkf obs25
frame obs24: use "${root}/${cleandata24}/obsrubric_lessonsids_2324.dta" 
frame obs25: use "${root}/${cleandata25}/obsrubric_lessonsids_2425.dta" 
...
drop id 
egen teachid = group(teacher partnersite year)

keep teachid year i16 teacher partnersite grade lesson year observer obsround i20 i23 i24 i26 i30 i32 i38 a b c i47 i48 i49 i50
...
global byvars teacher partnersite grade lesson year observer obsround
...
frame change rq1
	...
	collapse (sum) otr_*_* (mean) $descriptors_numeric $qualityvars  (firstnm) $descriptors_string , by($byvars otr_recipient)
	foreach var of varlist otr_*_* $descriptors_numeric $descriptors_string $qualityvars  {
	...
	collapse (mean) otr_*_*  $descriptors_numeric $qualityvars (firstnm) $descriptors_string , by(year obsround teachid otr_recipient)
	foreach var of varlist otr_*_* $descriptors_numeric $descriptors_string $qualityvars  {
		label variable `var' "``var'_label'"
	}
	...
	destring obsround , force replace
	
	tab otr_recipient , nolabel
	
	mvgstudy (otr_type_5 = teachid obsround|teachid) if otr_recipient ==3
	
	mvgstudy (otr_type_5 = teachid  obsround|teachid) if otr_recipient ==1
	
	
	mixed  otr_type_5 || teachid: if otr_recipient ==3, variance reml
	mixed  otr_type_5 || teachid: if otr_recipient ==1, variance reml
	mixed  otr_teachfeedback_2|| teachid: if otr_recipient ==3, variance reml
	...
	egen q = rowmean(quality*)
	
	
	mixed  otr_type_5 lessontime numstudents  || teachid: if otr_recipient == 3, variance reml
	mixed  otr_type_5 lessontime numstudents  || teachid: if otr_recipient == 1, variance reml
	mixed  otr_teachfeedback_2 lessontime numstudents || teachid: if otr_recipient == 3, variance reml
	
	
	
		
	mixed  otr_type_5 lessontime numstudents  || teachid: lessontime numstudents if otr_recipient == 3, variance reml //covariance(unstructured) 
	mixed  otr_type_5 lessontime numstudents  || teachid: if otr_recipient == 1, variance reml
	mixed  otr_teachfeedback_2 lessontime numstudents || teachid: if otr_recipient == 3, variance reml
	...
	sum otr_type_5 if otr_recipient == 1
	
	
	egen otr = rowtotal(otr_type_?)
	
	foreach var of varlist otr_type_? {
		gen prop_`var' = `var' / otr
	}
	
	sum prop* if otr_recipient ==1
		sum prop* if otr_recipient ==3
	
	
	
	mixed  otr_type_5 || teachid: if otr_recipient ==1, variance reml
	mat res1 = (r(table)[1,1] , r(table)[2,1]) \ (r(table)[1,2] , r(table)[2,2]) \ (r(table)[1,3] , r(table)[2,3])
	estat icc 
	mat res1 = res1 \ (r(icc2) , r(se2))
	
	mixed  otr_type_5 || teachid: if otr_recipient ==3, variance reml
	mat res2 = (r(table)[1,1] , r(table)[2,1]) \ (r(table)[1,2] , r(table)[2,2]) \ (r(table)[1,3] , r(table)[2,3])
	estat icc 
	mat res2 = res2 \ (r(icc2) , r(se2))
	
	mixed  otr_teachfeedback_2|| teachid: if otr_recipient ==3, variance reml
	mat res3 = (r(table)[1,1] , r(table)[2,1]) \ (r(table)[1,2] , r(table)[2,2]) \ (r(table)[1,3] , r(table)[2,3])
	estat icc 
	mat res3 = res3 \ (r(icc2) , r(se2))
	
	
	mat result rownames 
	mat result = res1, res2, res3
	
	
	
	
	
	frmttable , statmat(result) substat(1) ///
	ctitles(" ", "OTR for Non-LD Students" "OTR for LD Student" "Teacher Feedback To LD Student") /// 
	rtitles("Mean" \ "" \ "Between Teacher Variance" \  " " \ "Within Teacher Variance" \  " " \ "ICC" \ " ") 
	
	
	
capture program drop _one_spec
program define _one_spec, rclass
    // syntax: outcome recipient_label(if-value) title-label
    syntax varname numeric(min=1 max=1) [, RECIPient(integer 0) LABEL(string asis)]

    tempname mean var_bt var_wt icc
    quietly summarize `varlist' if otr_recipient == `recipient'
    scalar `mean' = r(mean)

    quietly mixed `varlist'  || teachid: ///
        if otr_recipient == `recipient', variance reml

    // mixed parameterization stores log SDs; convert to variances
    scalar `var_bt' = exp(2*_b[lns1_1_1:_cons])     // between-teacher variance
    scalar `var_wt' = exp(2*_b[lnsig_e:_cons])      // within (residual) variance
    scalar `icc'    = `var_bt'/(`var_bt' + `var_wt') // proportion due to teacher

    return scalar mean = `mean'
    return scalar var_bt = `var_bt'
    return scalar var_wt = `var_wt'
    return scalar icc = `icc'
    return scalar prop_within = 1 - `icc'
    return local  outcome = "`varlist'"
    return local  sample  = cond("`label'"=="","recipient==`recipient'","`label'")
end
